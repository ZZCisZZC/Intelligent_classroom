...
- **`esp32_mqtt/`:** **MQTT网关固件**。
  ...
  - **逻辑:** 在`setup()`中连接Wi-Fi和MQTT服务器。在`loop()`中：
    ...
    2.  监听来自S5PV210的串口数据。收到`UPDATE`指令后，调用`readFullJSON()`（一个带有括号匹配和超时机制的健壮的JSON读取函数）来获取完整的JSON，然后发布。
...
- **时间压缩:** 现实世界中的1秒模拟虚拟教室的10分钟，6秒模拟1个小时，便于快速验证
- **可控性:** 时间流逝完全可控，可以在设备关闭时保存，下次打开时延续。
...
- **历史数据:** 只在整点时刻将数据持久化到数据库，保障数据记录精度的同时减少I/O开销
- **功率计算:** 后端根据设备状态实时计算当前总功耗，包括灯光(LED)、空调、多媒体设备
